...
abund_mat <- read.csv(file = here::here("data", "static_changepoint.csv"), stringsAsFactors = F)
...
loadd(composite_ll_rdat_static_changepoint, cache = cache)

```

```{r plot static changepoint}

composite_ll_rdat_static_changepoint <- composite_ll_rdat_static_changepoint %>%
  mutate(k = as.factor(k),
         ncpt = as.factor(ncpt),
         seed = as.factor(seed))

all_var <- ggplot(data = composite_ll_rdat_static_changepoint, aes(x = k, y = sum_ll, color = seed)) +
  geom_boxplot() +
...
smooth_draws <- composite_ll_rdat_static_changepoint %>%
...
best_llplot <- ggplot(data = best_ll, aes(x = k, y = mean_llik, color = seed)) +
  ...
  facet_grid(rows = vars(form), cols = vars(ncpt), switch = "y") +
  theme_bw() +
  scale_color_viridis_d(begin = .01, end = .5) 
...
loadd(model_lls_2_4_0_time_dat_10L_rdat_static_changepoint, cache = cache)

plot(model_lls_2_4_0_time_dat_10L_rdat_static_changepoint$lda[[1]])
```


```{r plot ts}

plot(model_lls_2_4_0_time_dat_10L_rdat_static_changepoint$ts[[1]])

```
```{r generate species predictions, fig.height = 6, fig.width = 10}
source(here::here("fxns", "lda_wrapper.R"))
betas <- exp(model_lls_2_4_0_time_dat_10L_rdat_static_changepoint$lda[[1]]@beta)

thetas <- get_loo_theta(ts_model = model_lls_2_4_0_time_dat_10L_rdat_static_changepoint$ts[[1]], full_cov = data.frame(timestep = 1:30), sim = 50)
...
model_lls_2_4_0_time_dat_10L_rdat_static_changepoint$ts[[1]]$eta_summary
