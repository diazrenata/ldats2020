...
abund_mat <- read.csv(file = here::here("data", "static.csv"), stringsAsFactors = F)
...
loadd(composite_ll_rdat_static, cache = cache)

```

```{r plot static changepoint}

composite_ll_rdat_static <- composite_ll_rdat_static %>%
  mutate(k = as.factor(k),
         ncpt = as.factor(ncpt),
         seed = as.factor(seed))

all_var <- ggplot(data = composite_ll_rdat_static, aes(x = k, y = sum_ll, color = seed)) +
  geom_boxplot() +
...
smooth_draws <- composite_ll_rdat_static %>%
